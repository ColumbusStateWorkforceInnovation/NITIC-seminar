...
1. **The Radar Room**
   ...
   * **Game**: *k9s Speed Round*. The instructor deliberately injects a broken Deployment (`CrashLoopBackOff`) into everyone's namespace. The first student to find it using `k9s` and read the logs to diagnose the issue calls out the fix for the room!
