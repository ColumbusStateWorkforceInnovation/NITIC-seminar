...
3. **Paddling Out & Diagnostics (Lab 02)**
   ...
   * **Cluster Scavenger Hunt (Reconnaissance)**: The instructor hides a single Pod named `treasure-chest` in a randomly generated remote namespace. That pod continuously echoes a secret alphanumeric code in its logs. Students use global searches (`kubectl get pods -A`) to locate the hidden pod and read its logs to extract the code; share the code with the room when you find it.
