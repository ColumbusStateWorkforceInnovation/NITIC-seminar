...
### Key Objectives
- Install Oracle VirtualBox and build an Ubuntu VM from scratch.
...
2. **Lab 00 — Building Your Vessel**
   * **Mission**: Each student installs VirtualBox, creates an Ubuntu VM, installs Ubuntu, then runs `setup-client.sh` — one script that installs Fish, Starship, Docker, `kubectl`, `k9s`, and `aichat`, trusts the lab CA, and wires up `/etc/hosts`.
...
1. **The First Raft (Lab 01)**
   ...
   * **🤖 Curiosity Side-Quest — The Socratic Boatswain**: Mid-lab, students create an `AGENTS.md` file and configure the cluster's local LLM (Gemma) to act as a pedagogical TA that *refuses* to hand over copy-pasteable code. This is where the evolving AI persona used throughout the seminar is introduced.
