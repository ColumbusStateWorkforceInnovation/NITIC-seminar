...
- **Reference:** [`demo-kustomize.md`](demo-kustomize.md) (run-sheet + pre-flight), slides at [`slides/day-02/kustomize.md`](../../slides/day-02-kustomize.html), manifests in [`kustomize-demo/`](kustomize-demo/).
...
!!! tip "Running behind? Run the guided demo instead."
    Last session *no team finished Lab 03 in the slot*. If the room is behind, run
    the 30-min instructor-led walkthrough [`demo-fleet-logistics.md`](demo-fleet-logistics.md)
    (one keyboard, projector) instead of — or as a warm-up before — the solo lab.
    It builds the whole 3-tier stack, lands the silent-broken-link lesson, and
    gives **Gateway API / HTTPRoute** the deeper treatment the furthest teams got
    stuck on (pair it with the new [`gateway-api-personas.md`](../../slides/day-02-gateway-api-personas.html)
    slides — who-owns-what). The demo's manifests in `fleet-demo/` (`cache` /
    `backend` / `frontend`) are the **exact source the Day 3 Helm lab templatizes**,
    so the demo doubles as the Helm on-ramp.
