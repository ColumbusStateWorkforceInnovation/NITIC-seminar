...
- [ ] Add `git config --global user.email/user.name` to `setup-client.sh` (or as an explicit Day 3 Lab 02 step). Without it, the first `git commit` in Lab 02 blocks with "Please tell me who you are."
...
- [ ] **Verify aichat's AGENTS.md auto-load behavior.** Run `aichat` from a directory containing a known-distinctive AGENTS.md; confirm the role is picked up without an explicit `-r`/`--role` flag. If not, patch every "create AGENTS.md, then type aichat" line in the labs with the correct invocation.
...
- [ ] **Pre-test the Day 1 Mutiny Challenge.** Try 3–4 jailbreaks against State A on Gemma 3 4B ("ignore previous instructions," legitimizing-the-output, translation-task framing, two-part reply). Document which actually work — Eric needs known-good hints to feed the room at 4:55 PM Day 1 if no student finds one on their own.
