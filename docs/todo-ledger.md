...
- [ ] Add justfile recipe for **clabernetes** — values file exists in `k8s/core-tools/` but no `deploy-*` recipe; Day 3 Lab 03 needs it (currently called out as instructor-installed prereq).
  - [x] **chaos-mesh** done (2026-06-04): `just deploy-chaos-mesh` + the whole Day-4 attack toolkit — `normalize-repos` (Pirate force-pushes the fragile `island-stack` to every crew's maindeck + ensures their `<crew>-stack` ArgoCD app), `chaos-strike` (recurring pod-kill + pod-failure Schedules per namespace), `chaos-stress`, `chaos-status`, `chaos-calm` (kill switch), `teardown-chaos-mesh`. Canonical fragile chart in `k8s/chaos/island-stack/`; attack manifests in `k8s/chaos/`; `grade-cluster-recovery.sh` reconciled to `<crew>-frontend`/`<crew>-stack` naming. Day-4 docs rewritten to normalize-then-harden.
...
- [ ] **Bank canonical persona files in `agents-md/` (or `personas/`).** One file per state (`day-1-base.md`, `day-2-update.md`, `day-3-update.md`, `day-4-incident-commander.md`). Source of truth + `cp` recovery path for any student who falls behind. *(Started 2026-06-02: `examples/agents-md/day-1-the-salty-boatswain.md` exists + Day-1 lab transcludes it via `pymdownx.snippets` `--8<--`. Remaining states still inline in their lab docs.)*
- [ ] **Two-stage Boatswain: "unhelpful by design" → helpful upgrade.** *(Decided 2026-06-02.)* The Day-1 Boatswain is DELIBERATELY unhelpful — a strict Socratic refuser (no copy-pasteable code; explains `FROM`/`COPY` conceptually only). That preserves the Day-1 "watch him refuse" beat and makes the Mutiny jailbreak meaningful. Partway through the course students UPGRADE him to a genuinely helpful version that gives complete, explained code — the `cp`-overwrite-AGENTS.md pattern, same mechanics as the Day-4 Incident Commander swap. The payoff: students *feel* the difference and learn that the `AGENTS.md` context is what governs the assistant's behaviour.
    - Helpful "upgrade" version authored + committed: `examples/agents-md/day-1-the-salty-boatswain.md` (commit `9a9288c` — qwen3-tuned, strict opener rotation, Review mode, gives real explained code). NOTE: filename says "day-1" but it is really the UPGRADE state — rename once the swap point is chosen.
    - TODO: author the "unhelpful by design" Day-1 base as its own file in `examples/agents-md/` — the strict-refusal version (pull from the pre-`9a9288c` "one hard rule / bracketed-skeletons-only" persona in git history, or the original 2026-05-31 prompt). Day-1 lab should transclude THAT, not the helpful one.
    - TODO: decide WHERE the upgrade lands (which day/lab) and write the swap step (overwrite `~/lab/AGENTS.md`, `.exit`, re-`hail`), mirroring the Day-4 Incident Commander mechanics.
    - TODO: reconcile lab narratives once the base/upgrade split is in place — Day-1 Step 1 "refuses exact code" (true for base), Mutiny jailbreak (true for base), Day-4 Socratic→Commander contrast (revisit now that a mid-course helpful upgrade exists).
    - Test loop now exists: `just test-boatswain [FILE] [Q]` / `just test-boatswain-convo [FILE]` (`scripts/test-persona.py`) sends any persona file to the live model + auto-checks (opener rotation, `<think>` leak, whether real code was shown). Use it to validate BOTH states.
    - Gotcha (cost an evening): `hail` loads `~/lab/AGENTS.md` (symlinked role file), which silently goes stale vs the repo/docs copies. Always `cp` the canonical file over it + restart `hail` before testing.
